 ...
 *   - Only automationtodayca@gmail.com is authorized.
 *   - ATD_SECRET remains server-side in Script Properties.
 *   - No secret is included in GitHub/frontend.
 */
...
function doGet() {
  return json_({
    ok: true,
    service: 'AutomationTodayCA Service Report',
    status: 'active'
  });
}
...
function doPost(e) {
  try {
    ...
    // ATD_SECRET is intentionally kept server-side.
    // Google authentication is the browser-to-backend authentication mechanism.
    const serverSecret = PropertiesService.getScriptProperties().getProperty('ATD_SECRET');
    if (!serverSecret) {
      throw new Error('ATD_SECRET is not configured.');
    }
    ...
    if (authenticatedEmail !== ALLOWED_GOOGLE_EMAIL) {
    ...
    let bytes;
    try {
      bytes = Utilities.base64Decode(pdfBase64);
    } catch (err) {
    ...
    return json_({
      ok: false,
      error: String(err && err.message ? err.message : 'Request could not be completed.')
    });
  }
}
...
function verifyGoogleCredential_(credential) {
  ...
  try {
    ...
    if (response.getResponseCode() !== 200) {
      log_('Google tokeninfo HTTP ' + response.getResponseCode());
      return null;
    }
    ...
    log_('Google verification error: ' + (err && err.message ? err.message : err));
    return null;
  }
}
...
function uniqueEmails_(values) {
  return [...new Set(values.filter(Boolean).map(function(v) {
...
function sanitizeFilename_(filename) {
  const cleaned = String(filename)
    ...
    .trim()
    .substring(0, 180);
  return cleaned || 'service-report.pdf';
}

function log_(message) {
  console.log('AutomationTodayCA: ' + message);
}
