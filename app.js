...
function getSectionStatus(section,index){
  ...
  // Dynamic email/phone rows are intentional empty slots until the user types into them.
  // Only the primary email is required; an added empty row does not make the section yellow.
  const optional=[...section.querySelectorAll("input:not([required]):not([type=hidden]),select:not([required]),textarea:not([required])")].filter(el=>{
    if(el.closest(".locked")) return false;
    if(el.disabled) return false;
    if(el.closest(".email-row") || el.closest(".phone-row")) return fieldHasValue(el);
    return true;
  });

  const optionalMissing=optional.some(el=>!fieldHasValue(el));
  return optionalMissing ? "yellow" : "green";
}
...
function addCustomerPhone(){
  const container=$("#customerPhones");
  if(!container) return;
  const row=document.createElement("div");
  row.className="phone-row";
  row.innerHTML='<input type="tel" name="customerPhoneExtra[]" placeholder="e.g. 647-555-1234"><button type="button" class="remove-phone" onclick="removeCustomerPhone(this)">×</button>';
  container.appendChild(row);
  bindFormStatusListeners(row);
  updateProgress();
}

function removeCustomerPhone(btn){
...
function updateRequiredFieldHighlights(){
  document.querySelectorAll("#serviceForm input[required], #serviceForm select[required], #serviceForm textarea[required]").forEach(el=>{
    const missing=!fieldHasValue(el);
    el.classList.toggle("required-missing",missing);
    const label=el.closest("label");
    if(label) label.classList.toggle("required-missing-label",missing);
  });
  const sigWrap=document.querySelector(".signature-wrap");
  if(sigWrap) sigWrap.classList.toggle("required-missing-block",!hasSig);
}
...
function collect(){
 ...
 o.customerPhones=[...document.querySelectorAll('#customerPhones input[type="tel"]')].map(i=>String(i.value||'').trim()).filter(Boolean);
...
function renderReview(o,finalized=false){
 ...
 <div class="review-grid">
   ...
   <div><span>Phone(s)</span><strong>${escapeHtml((o.customerPhones||[o.phone||""]).join(", ")||"—")}</strong></div>
...
async function generatePDF(saveFile=true){
 try{
  ...
  const pdfPhones=(o.customerPhones||[o.phone||""]).filter(Boolean).join(", ");
  field(M,y,58,"Phone(s)",pdfPhones);field(M+61,y,88,"Service Address",o.address);field(M+152,y,32,"City",o.city);y+=16;
